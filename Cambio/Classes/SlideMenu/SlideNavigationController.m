...
- (void)setup
{
    ...
    self.landscapeSlideOffset = self.view.frame.size.width*0.6;//MENU_DEFAULT_SLIDE_OFFSET;
	self.portraitSlideOffset = self.view.frame.size.width*0.6;//MENU_DEFAULT_SLIDE_OFFSET;
	self.panGestureSideOffset = 0;
	self.avoidSwitchingToSameClassViewController = YES;
	self.enableShadow = YES;
	self.enableSwipeGesture = NO;//jintest
	self.delegate = self;
}
